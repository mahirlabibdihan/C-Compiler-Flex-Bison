...
void SemanticAnalyzer::defineFunction(string ret_type, string id_name, vector<Variable *> params)
{
    ...
    for (auto p : params)
    {
        new_func->addParam(p->getDataType());
    }
    ...
    else
    {
        ...
        asm_gen->startProcedure(new_func);
    }
}
...
string SemanticAnalyzer::callFunction(string id_name, vector<string> args)
{
    // cout << id_name << endl;
    Identifier *id = (Identifier *)table->search(id_name);
    if (id == NULL)
...
void SemanticAnalyzer::declareVariable(string data_type, string var_name)
{
    ...
    else
    {
        ...
        // cout << var->getOffset() << endl;
    }
}
void SemanticAnalyzer::declareArray(string data_type, string arr_name, int arr_size)
{
    Array *var = new Array(arr_name, data_type, arr_size);
...
string SemanticAnalyzer::assignOp(Variable *left, Expression *right)
{
    // cout << left << endl;
