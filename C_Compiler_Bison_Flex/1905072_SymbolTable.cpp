...
string SymbolTable::getScopeIdOfSymbol(const string &key)
{
    ScopeTable *cur = current_scope;
    while (cur != nullptr)
    {
...
pair<int, int> SymbolTable::getLocationOfSymbol(const string &key)
{
    return current_scope->getLocationOfSymbol(key);
}
